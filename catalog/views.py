...
class IsArtistOwnerOrReadOnly(permissions.BasePermission):
    def has_object_permission(self, request, view, obj):
        ...
        if request.method in SAFE_METHODS:
            return True
        # записи только владельцу-артисту
        return (
...
class ArtworkViewSet(viewsets.ModelViewSet):
    ...
    # точные фильтры
    filterset_fields = {
        "categories__slug": ["exact"],  # ?categories__slug=portrait
        "price": ["gte", "lte"],        # ?price__gte=10&price__lte=100
        "artist__id": ["exact"],        # ?artist__id=3
        "is_active": ["exact"],         # см. get_queryset ниже
    }
    # полнотекстовый и по полям
    search_fields = ["title", "description", "tags", "artist__display_name"]
    ordering_fields = ["created_at", "price", "title"]  # ?ordering=-price

    def get_queryset(self):
        qs = super().get_queryset()
        # Публичный список — показываем только активные
    ...
    def perform_create(self, serializer):
        ...
        serializer.save(artist=self.request.user.artist, is_active=False)  # на модерацию
