...
class CartView(views.APIView):
    ...
    def get(self, request):
        cart = Cart(request)
        # items() уже должен возвращать сериализуемые dict'ы
...
class CheckoutView(views.APIView):
    """
    Простой чекаут: создаём оплаченный заказ, генерим download-токены, чистим корзину.
    """
    permission_classes = [permissions.AllowAny]
...
# Ниже — только чтение заказов в «Мои заказы» (если используешь).
